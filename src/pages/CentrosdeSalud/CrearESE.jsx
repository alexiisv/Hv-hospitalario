...
import { crearEse } from "../../services/eseService";

export default function CrearESE() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      alert("ESE creada correctamente");
      reset();
      navigate("/eses");
    } catch (error) {
      console.error("Error al crear ESE:", error);
      alert("Ocurrió un error al crear la ESE");
    } finally {
      setGuardando(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl shadow-sm border p-6">
        <h2 className="text-2xl font-bold text-slate-800">Crear ESE</h2>
        <p className="text-sm text-slate-500 mt-1">
          Registra una institución para asociarle inventario biomédico.
        </p>
      </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="Ej: E.S.E. CENTRO DE SALUD SANTIAGO APÓSTOL"
            />
          </div>
        ...
        <div className="flex justify-end gap-3 border-t pt-6">
          ...
          >
            {guardando ? "Guardando..." : "Guardar ESE"}
          </button>
        </div>
      </form>
    </div>
